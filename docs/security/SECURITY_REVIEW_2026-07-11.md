...
| SEC-007 | P2 | `app/api/contact/route.ts` | The public contact endpoint accepted arbitrary browser origins and waited indefinitely for a configured delivery provider. | Require a same-origin JSON request, cap declared request size, constrain webhook configuration to credential-free HTTPS URLs, and time-box external delivery requests. | Unit contract test plus HTTP probes confirm `403` for a foreign origin, `415` for non-JSON, `413` for an oversized body, and an honest `503` fallback when no delivery provider is configured. | Header-based origin checks do not replace a distributed rate limit; deploy a managed edge/WAF rule or shared store before enabling high-volume delivery. |
| REL-001 | P1 | `services/error.tsx` | The error boundary called `window.location.reload()` after every caught rendering error; its session flag was not read anywhere else, so a failing device could enter a reload loop. | Remove forced reload and use Next's boundary `reset()` only after an explicit visitor action. | Source contract, production build, and mobile route smoke pass. | A rendering error still falls back to the boundary screen; browser/GPU crash recovery must also be tested on physical devices. |
| REL-002 | P2 | `public/fonts/quantify/Quantify-Bold.ttf` | Asset cleanup removed a font still referenced by `.font-numbers`, creating a production `404` and possible visual fallback on numeric hero content. | Restore the exact tracked font file and retain it as a runtime dependency. | Local production server returns `200 font/ttf`; hash matches the pre-cleanup revision. | Keep asset-reference checks in cleanup reviews. |
...
| RES-005 | P2 | Mobile animation workload | A synthetic mobile CPU x4 scroll run still reports sub-45 FPS on the animation-heavy home, about, services, and mission routes, although it found no crash, page error, failed asset, or horizontal overflow. | Profile on representative physical iOS/Android devices; reduce or pause nonessential reveal/Lottie work where a real-device trace confirms a bottleneck. |
