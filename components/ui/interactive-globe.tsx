...
// Customized for DDC: Astana (HQ), Almaty (DC), and global hubs
const DEFAULT_MARKERS = [
  { lat: 51.1694, lng: 71.4491, label: "Astana (HQ)" },
  { lat: 43.2220, lng: 76.8512, label: "Almaty (Processing)" },
  { lat: 51.5074, lng: -0.1278, label: "London Gateway" },
  { lat: 50.1107, lng: 8.6821, label: "Frankfurt Gateway" },
  { lat: 1.3521, lng: 103.8198, label: "Singapore Node" },
  { lat: 35.6762, lng: 139.6503, label: "Tokyo Node" },
  { lat: 37.7749, lng: -122.4194, label: "San Francisco Node" },
];

const DEFAULT_CONNECTIONS: { from: [number, number]; to: [number, number] }[] = [
  { from: [51.1694, 71.4491], to: [43.2220, 76.8512] }, // Astana - Almaty
  { from: [51.1694, 71.4491], to: [50.1107, 8.6821] },  // Astana - Frankfurt
  { from: [51.1694, 71.4491], to: [51.5074, -0.1278] },  // Astana - London
  { from: [43.2220, 76.8512], to: [1.3521, 103.8198] },  // Almaty - Singapore
...
export function InteractiveGlobe({
  className,
  size = 600,
  dotColor = "rgba(82, 183, 136, ALPHA)", // Default Forest Green opacity sampled
  arcColor = "rgba(232, 200, 122, 0.4)", // Gold transparency
  markerColor = "rgba(232, 200, 122, 1)", // Gold marker
  ...
  const draw = useCallback(() => {
    ...
    // Outer glow (green/blue ambient glow)
    const glowGrad = ctx.createRadialGradient(cx, cy, radius * 0.8, cx, cy, radius * 1.5);
    glowGrad.addColorStop(0, "rgba(82, 183, 136, 0.03)");
...
// Export as "Component" as requested in task description
