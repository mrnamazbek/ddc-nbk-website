    ...
    <div className={`relative flex w-full overflow-hidden py-4 select-none [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)] ${className}`}>
      ...
      >
        {marqueeItems.map((item, idx) => (
          <div
            key={idx}
            className="flex items-center justify-center px-6 py-3 rounded-full border border-glass-border bg-white/[0.02] backdrop-blur-md text-foreground/60 font-mono text-sm tracking-widest hover:text-foreground hover:border-brand-blue-light/30 hover:bg-brand-blue/5 transition-all duration-300"
          >
            {item}
          </div>
        ))}
