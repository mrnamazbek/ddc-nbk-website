...
import { useScroll, useTransform, useInView, useReducedMotion, motion } from "framer-motion";
  ...
  return (
    <div ref={itemRef} className="flex justify-start pt-10 md:pt-32 md:gap-10">
      ...
      >
        <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-background flex items-center justify-center border border-glass-border shadow-[0_0_10px_rgba(232,200,122,0.15)]">
          <motion.div
            ...
            animate={!reduce && isActive ? { scale: [1, 1.35, 1], opacity: [0.75, 1, 0.75] } : { scale: 1, opacity: 0.6 }}
            transition={!reduce && isActive ? { duration: 2, repeat: Infinity, ease: "easeInOut" } : { duration: 0.4 }}
          />
        </div>
        <motion.h3
      ...
      <div className="relative pl-20 pr-4 md:pl-4 w-full">
        ...
        <motion.div
          initial={reduce ? false : { opacity: 0, x: fromSide, filter: "blur(10px)" }}
...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  // Grow vertical line height on scroll — reduced-motion users get the fully
  // drawn line immediately instead of it filling in as they scroll.
      ...
      <div ref={rectRef} className="relative max-w-7xl mx-auto pb-20">
        ...
        >
          <motion.div
            style={
              reduce
                ? { height: containerHeight, opacity: 1 }
