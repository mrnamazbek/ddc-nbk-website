...
type PatchedConsoleMethod = typeof console.error & {
  __react19ThemesPatched?: boolean;
  __threeDeprecationsPatched?: boolean;
};
...
if (typeof console !== "undefined") {
  if (console.error) {
    const errorMethod = console.error as PatchedConsoleMethod;
    const isPatched = errorMethod.__react19ThemesPatched;
    if (!isPatched) {
      ...
      patched.__react19ThemesPatched = true;
  ...
  if (console.warn) {
    const warnMethod = console.warn as PatchedConsoleMethod;
    const isWarnPatched = warnMethod.__threeDeprecationsPatched;
    if (!isWarnPatched) {
      const originalConsoleWarn = console.warn;
      const patchedWarn: PatchedConsoleMethod = (...args: unknown[]) => {
        const first = args[0];
        const msg = typeof first === "string" ? first : first instanceof Error ? first.message : "";
        if (
      ...
      patchedWarn.__threeDeprecationsPatched = true;
